...
- **PR-G3 merged 2026-07-19** (@3b0a6f6): retention-aware forgetting — SM-2 per reviewed item (`ease`/`review_interval_days` on `session`), per-item 0–5 grades in the review close, eviction by `TUTOR_REVIEW_HORIZON_DAYS` (60) instead of count, Ebbinghaus retention shown in "Tu progreso"/`GET /memories`. **Feature G complete** ⇒ milestone dogfood point. One audit fix pre-merge: malformed grades schedule as q=3 but never evict (contract letter), regression-locked.
- **Dogfood tracker:** `Proyecto-Atenea/DOGFOOD.md` (outside the repo, next to `context_files/`) — agents add items with exact steps at merge; the developer fills in verdicts. Check it on arrival; move closed items to its history section.
- **Verification state:** 160 tutor tests + `make smoke` (15-step fake-LLM journey, <1s, zero keys) + ruff + mypy green on `main`; GitHub CI green (ci-logs channel silent). **Pending on the developer:** live-stack validation of migration 23 + real grounded search (`TUTOR_SMOKE_BASE_URL` run or a browser session on an indexed source), and a milestone browser dogfood.
...
- **Eval loop LIVE (2026-07-19 night, PR-DX3 + secret + 5 runs):** `Tutor Eval` workflow runs the E2 harness in CI on dispatch or any push touching `tutor/prompts/**` / `tutor/eval/**`; every run publishes JSON + judge table to the accumulating **`eval-reports` branch** (read via `git fetch origin eval-reports`); failures land on `ci-logs`. **Baseline 0.55/2** (v2 prompt) → **prompt v3** (@a0a494f: operational technique playbook IN the prompt, never-solve-the-live-task with teach-at-impasse counterweight, micro-consolidation, tutor-owned close) sampled **0.9 / 0.65 / 0.8**. Three instrument fixes shipped en route (pipefail; personas signal + answer the close; rubric judges the close ATTEMPT with verbatim quote). **Iteration discipline (developer decision): judge stays DeepSeek for now (hallucination-prone = good testbed) and N≥2 runs per configuration before accepting any delta (same-config variance observed ±0.25).**
- **Next queue:** (1) **W1 contract** — per-turn verification of tutor replies, grounded-first; design in `hallucination_verification_sota.md` §5 + §6 addendum (2026 frontier is white-box/API-unreachable ⇒ leverage = context engineering + black-box claim-check + abstention; NotebookLM case: whole-source grounding + closed-world contract + mandatory citations ⇒ ~13% vs ~40% hallucination). **Open developer decisions before contracting: synchronous gate vs async correct-next-turn (latency), and whether M3's whole-source injection path (NotebookLM-shaped) joins W1's scope or stays in M3.** (2) more measured prompt iterations under the N≥2 rule. Parked: PR-M3 (signed), M-tasks, T, V; K + memory-informed M-tasks blocked by the sequencing rule until DOGFOOD item 2. Parked until the dogfood era: PR-M3 (signed, §1), M-tasks, T, V; K/tree and memory-informed M-tasks additionally blocked by the sequencing rule. **Sequencing rule (developer, 2026-07-19):** nothing that *consumes* learner-memory content gets contracted until the developer has dogfooded Feature G (DOGFOOD.md item 2). Contracts per the standing protocol (architect pins facts → Sonnet implements → architect audits).
