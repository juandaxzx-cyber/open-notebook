...
- **Measured pedagogy loop LIVE:** `Tutor Eval` workflow (dispatch, or push touching `tutor/prompts/**` / `tutor/eval/**`) replays 4 personas through the real tutor LLM and an LLM judge (10 evidence criteria, scale 0–2). Baseline (v2 prompt) 0.55 → v3 sampled 0.9 / 0.65 / 0.8. **Standing discipline: judge stays DeepSeek (developer decision — hallucination-prone testbed) and N≥2 runs per configuration before accepting any delta (same-config variance ±0.25).**
- What runs today: `docker compose up -d --build` = SurrealDB + OpenNotebook (8502/5055) + tutor (5056: chat UI, questionnaire, source picker, resume, history, review loop, "Tu progreso" with retention decay). Dev loop: `make tutor-dev` / `make tutor-restart`; instant check: `make smoke`; full gate: `make check-tutor`; eval: `make eval-tutor`.
- Decisions fixed: tutor service in **`tutor/`**; Python **3.12** via `uv`; separate **`atenea`** SurrealDB database; `TUTOR_USER_ID` (default `juanda`); LLM access only via the Esperanto-wrapping interface (+ deterministic `fake` provider); repo name stays `open-notebook` (rename = open decision). Credentials: GitHub PAT (workflow scope) + DeepSeek key in use; repo secret `DEEPSEEK_API_KEY` powers CI evals; the PAT may be passed to agents via chat (AGENTS.md credentials-channel amendment, 2026-07-19).
- **Dogfood: deferred for logistics, absolute commitment** — `Proyecto-Atenea/DOGFOOD.md` (outside the repo) lists every pending item with exact steps; agents add items at merge, the developer fills verdicts. **Sequencing rule (developer, 2026-07-19): nothing that consumes learner-memory content (K/tree, memory-informed M-tasks) gets contracted until DOGFOOD item 2 closes.**
- **Next queue:** (1) **PR-W1** — per-turn verification of tutor replies, grounded-first. **Pre-contract decisions resolved by the developer 2026-07-19: (a) synchronous gate with escalation ladder — fail ⇒ retry ⇒ stronger model (verifier-as-generator, ≤2) ⇒ best answer incl. honest limits-admission; every fail/escalation logged; default high-verification profile + a cheap profile with explicit more-errors caveat; (b) whole-source-lite in W1 (budget switch inside `retrieve_grounding`; sectionize/coverage stay M3).** **Contract SIGNED 2026-07-19 (§1) — next step: implementation on `feature/w/verify-turn`.** Design basis: `hallucination_verification_sota.md` §5 + §6 (closed-world contract, mandatory citations, whole-source evidence). (2) Further prompt iterations under the N≥2 rule. Parked until the dogfood era: PR-M3 (signed, §1), M-tasks, T, V, plus the sequencing-rule blocks above.
...
### PR-W1 — Per-turn verification of tutor replies *(Feature W; contract SIGNED OFF by the developer 2026-07-19 — ready for implementation)*

Generalizes G2's fenced verification to the live tutoring turn — the learner-facing gap in `hallucination_verification_sota.md` §4. Grounded-first (§5) + the NotebookLM lesson (§6): closed-world contract + mandatory citations + whole-source evidence when it fits.

**Developer decisions (2026-07-19):**

- **(a) Synchronous gate with an escalation ladder (clarified by the developer 2026-07-19; supersedes an earlier misreading).** Verify-before-send. Default = **high-verification profile**: verify fails ⇒ the generator retries once (violations injected as constraints) ⇒ re-verify ⇒ fails again ⇒ **escalate to a stronger model** (the verifier acting as generator — Feature W's core pattern) for up to 2 attempts, each re-verified ⇒ all attempts fail ⇒ ship the **best possible answer, which at times is honestly admitting the session's knowledge limits** (abstention as first-class outcome, never a stall). **Every failed verification and every escalation is persisted** — the record is data. A **cheap profile** must also exist (explicit product caveat: cheaper ⇒ more errors): single verify, on fail one regeneration, still failing ⇒ send with a visible uncertainty flag, no escalation.
- **(b) Whole-source-lite in W1 — leaning on OpenNotebook's existing machinery (developer directive 2026-07-19: reuse what ON does well; rework only after exploration).** Explored 2026-07-19: ON already implements whole-source-under-token-budget — `open_notebook/utils/context_builder.py::build_source_context(source_id, max_tokens)` (source always kept, insights dropped LIFO to fit; source-chat uses 50k tokens; counting via tiktoken `o200k_base`). The tutor consumes this over REST (API-first): `retrieve_grounding` internals switch — source fits `TUTOR_GROUNDING_BUDGET_TOKENS` (default 16000, provisional) ⇒ inject whole source (client gains `get_source(id)` over the REST `full_text`, pulled forward from M3; budget in TOKENS per ON convention, tutor-side estimate acceptable); else ⇒ today's scoped retrieval. Signature stable (M1→M2 pattern), engine + existing tests untouched. Rationale: a verifier over partial top-k evidence produces false fails on claims that ARE in the source. Sectionize + coverage remain PR-M3 (parked).

**One seam — new module `tutor/session/verification.py`:**

- **No skip heuristic (developer, 2026-07-19):** every tutor reply within scope is verified — a failed verification is signal to collect for iterative improvement, not a cost to avoid. Cost control lives in scope (`TUTOR_VERIFY_TURNS`) and profile (`high`/`cheap`) only.
- `verify_turn(verifier_llm, reply, evidence, settings) -> TurnVerdict` — grounded: RAGAS-faithfulness-shaped claim check against the injected evidence + citation check (each [n] must match what passage/section n says — mis-citation is the education field's worst failure). Ungrounded: no pretend-verification — overclaiming/fabricated-reference + abstention check only.
- Escalation ladder (decision (a); `escalate(...)` in the same module): **high** — gen-retry(1, violations as constraints) → verifier-as-generator(≤2), each attempt re-verified → all fail ⇒ best-available answer, with honest admission of limits when the evidence doesn't support a confident one. Worst case ≈ 3 regenerations + 4 verifier calls per gated turn — contained by scope + profile only (no skip heuristic). **cheap** — verify(1) → gen-retry(1) → still fail ⇒ send flagged, no escalation; the UI surfaces the cheap-mode caveat ("más barato ⇒ más errores"). Malformed verifier JSON ⇒ send unflagged + log (instrument failure never loses a turn — G3's parse-error principle). Every fail/escalation/outcome persisted (below).

**Engine edit:** every outgoing tutor reply routes through the seam at exactly one integration point; removing the call reverts the feature.

**Prompts:** `verify_turn.md` (verifier); `session_system.md` gains a closed-world + mandatory-citations section for grounded sessions (prompt change ⇒ DX3 eval fires; this is prompt v4 territory, N≥2 rule applies). **Citation format aligned with ON's existing convention** (`prompts/source_chat/system.jinja`, `ask/final_answer.jinja`): bracketed record IDs, "never make up IDs, use them exactly as provided" — whole-source mode cites `[source:id]`; scoped mode keeps per-passage `[n]` markers, each resolvable to its source record ID (passage-level anchors are what the verifier checks). ON has the citation *contract* but no verifier — W1 supplies exactly the missing half, no ON rework needed.

**Config (env, `.env.example` names only):** `TUTOR_VERIFY_TURNS = off | grounded | all` (scope; default `grounded`); `TUTOR_VERIFY_PROFILE = high | cheap` (default `high`); reuses `TUTOR_VERIFIER_PROVIDER/MODEL` (G2; unset ⇒ tutor's LLM + same-model warning) as the escalation generator — env names fixed here, **but WHICH models fill generator/verifier roles is an OPEN developer decision** (2026-07-19): big models are costly at scale, cheap DeepSeek hallucinates too much; the tradeoff analysis is pending and the persisted verification traces + DX3 eval loop are its designated evidence base. `TUTOR_GROUNDING_BUDGET_TOKENS` (default 16000, provisional) for the whole-source switch.

**Persistence:** transcript entries gain optional `verification` metadata (verdict, violations per attempt, attempts/escalation trace, final outcome: `clean | corrected | escalated | limits-admitted | flagged`) — additive, atenea DB only; this is the developer's "debemos mantener el registro" and the raw material for measuring verifier quality later. **UI:** flagged/limits turns render a visible uncertainty notice; cheap profile shows its standing caveat (chip, Spanish-first).

**Fake provider:** deterministic verdicts (pass / fail-then-pass / fail-fail-escalate-pass / all-fail fixtures); smoke gains a verified-turn step, stays zero-key.

**Tests:** ladder paths per profile (high: pass ⇒ send; fail ⇒ retry-pass; fail×2 ⇒ escalated-pass; all-fail ⇒ limits-admitted send · cheap: fail ⇒ retry ⇒ flagged send; malformed ⇒ unflagged send + log); citation-mismatch detection; whole-source switch (fits ⇒ full text / not ⇒ scoped / `TUTOR_VERIFY_TURNS=off` and no-grounding ⇒ byte-identical lock tests, M1-style); ungrounded abstention path; verification trace persists. No network/DB/real LLM.

**Merge gate — first eval-gated PR:** DX3 before/after under the N≥2 discipline: error-flagging, real-checks, calibration, non-sycophancy must not regress; mis-citations ~0 on the persona suite.

**Delimitation:** diff confined to `tutor/` + `tests_tutor/` (+ `.env.example` names, docs mirror). Out of scope: async correct-next-turn (later W slice), sectionize/coverage (M3), trained verifiers, sampling/semantic-entropy detection.

**Usable when:** in a grounded session a wrong citation never reaches you silently — it arrives corrected (retry or escalation), or as an honest admission of limits, or visibly flagged (cheap profile); the verification trace of any gated turn is inspectable in the session record; `TUTOR_VERIFY_TURNS=off` restores today's behavior byte-for-byte; the eval before/after shows no pedagogy regression and ~0 mis-citations.

**W1-eval addendum (contract amendment — architect, developer-approved 2026-07-19; renegotiation note: the developer explicitly authorized architects to amend signed contracts for technical improvements):** audit found the DX3 merge gate vacuous as written — the four E2 personas run ungrounded with verification off, so they exercise neither the closed-world prompt nor the gate. Amendment, same branch (`feature/w/verify-turn`), before merge:

- **Fifth persona `05_grounded_miscitas.json`.** `Persona` gains additive optional fields `source_id: str | None = None`, `source_text: str = ""` (existing personas untouched — lock test). The persona carries a short factual source text (fits the 16k-token whole-source budget by design) with verifiable specifics (numbers, dates, named entities), plus turns that bait overreach: at least one question the source answers, one it does NOT answer (tests honest limits vs. fabrication), and one inviting a plausible-but-wrong citation. Turn `note` annotations state which claims the source supports — the judge's ground truth.
- **Fake registry:** when `persona.source_id` is set, `build_fake_registry` also registers `content.get_source` returning `{full_text: source_text, title: name}` — the whole-source-lite path is what the eval exercises (scoped fallback stays untested here; it has unit tests).
- **Runner:** `run_persona_session` opens with `source_id=persona.source_id` and constructs the engine with `grounding_enabled=bool(persona.source_id)`, `verify_turns`/`verify_profile`/`grounding_budget_tokens` from `TutorSettings.from_env()`, and a verifier resolved like the service does (`TUTOR_VERIFIER_PROVIDER/MODEL`, else tutor LLM + warning). `TUTOR_VERIFY_TURNS` therefore switches the eval between baseline (`off`) and gated (`grounded`) runs — the before/after lever.
- **Measurement (both additions reported OUTSIDE the 10 criteria means, preserving cross-version comparability):** (1) programmatic, bias-immune `invented_citations` metric per persona — citation-like markers in tutor turns that cannot resolve (`[n]`/foreign `[source:x]` in whole-source mode; any citation marker for ungrounded personas); (2) judge-scored `citation_check` (0–2, grounded personas only, reference-anchored to `source_text`): do cited passages actually support the claims next to them? (3) per-persona aggregation of persisted verification outcomes/attempts from the transcript traces.
