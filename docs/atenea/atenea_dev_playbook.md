...
- **Verification state:** 107 tutor tests + `make smoke` (12-step fake-LLM journey, <1s, zero keys) + ruff + mypy green on `main`. **Pending on the developer:** live-stack validation of migration 23 + real grounded search (`TUTOR_SMOKE_BASE_URL` run or a browser session on an indexed source), and a milestone browser dogfood.
