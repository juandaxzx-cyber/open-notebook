...
- **Merged 2026-07-14 (V1 policy gate: checks + smoke):** PR-DX2 (f18aed1 — `TUTOR_LLM_PROVIDER=fake`, `make smoke` 12-step journey <1s, `make tutor-dev`/`tutor-restart`, smoke in CI) and PR-F3 (fb30025 — in-UI profile questionnaire with 409→form→retry, grounding picker behind `grounding_enabled`, provider/model chip). 101 tests + smoke green post-integration.
- **Merged 2026-07-14 (V1 gate):** PR-M2 (0f10159 — source-scoped `fn::vector_search` via migration 23(+down), `source_id` threaded through domain/API, tutor seam sends it server-side with client-side filter kept as defense-in-depth; CORE_CHANGES.md entry with revert recipe; CI-only pydantic guard in `tests/`). 107 tests + smoke green. **Live-stack validation pending (developer):** migration 23 execution + real grounded search — `TUTOR_SMOKE_BASE_URL` run or a browser session on an indexed source. **Next:** PR-G2+ (learner memory, see `agent_memory_sota.md`), M-coverage/M-tasks, T/V registered.
