...
- **Next queue:** (1) **W1 contract** — per-turn verification of tutor replies, grounded-first; design in `hallucination_verification_sota.md` (§5 recommendation + §6 addendum: 2026 frontier is white-box/API-unreachable ⇒ leverage = context engineering + black-box claim-check + abstention; NotebookLM case = whole-source grounding + closed-world contract + mandatory citations). **Developer decisions owed before contracting: (a) synchronous gate vs async correct-next-turn, (b) whole-source grounding in W1 or M3.** (2) Further prompt iterations under the N≥2 rule. Parked until the dogfood era: PR-M3 (signed, §1), M-tasks, T, V, plus the sequencing-rule blocks above.
