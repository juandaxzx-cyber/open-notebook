...
- **Next in queue:** PR-R1 (session resume) — implemented @ `c8a0fa2` on `feature/r/session-resume`, pending dogfood + merge. **PR-M1 (material grounding) implemented @ `681197a` on `feature/m/grounding`** (branched off the R1 tip). Next: developer review of R1 + M1, then PR-M2 (core, after the §4 upstream sync), then Feature G onward + registered T/V in `atenea_pr_plan.md`.
- **⚠ Autonomous batch (2026-07-14, reduced-review window):** PR-M1 was implemented end-to-end without step-by-step developer review, isolated on `feature/m/grounding`. It is **opt-in, off by default** — set `TUTOR_GROUNDING_ENABLED=1` to try it; unset, sessions behave exactly as before. The PR-M1 hard invariant held: the code diff is confined to `tutor/` + `tests_tutor/` — **zero edits under `open_notebook/`, `api/`, `migrations/`** (verify: `git diff main...feature/m/grounding --stat`). `make check-tutor` green (76 tests, ruff, mypy). Dogfood + review before merging; `TUTOR_GROUNDING_ENABLED` is documented in `tutor/config.py` (no `.env.example` edit, to keep the invariant).
