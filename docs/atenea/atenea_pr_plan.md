...
**Everything delivered is merged to `main` and pushed**: V1 (PR-0 + A–F), DX1, E2, F2, R1, H1, upstream sync (@7dfe8aa), DX2, F3, Feature M through M2, and **Feature G complete** (G1 + G2 + G3, G3 @3b0a6f6) — 160 tutor tests + 15-step `make smoke` + ruff + mypy green, GitHub CI green (failures agent-readable via the `ci-logs` branch; eval history will live on `eval-reports`). V1-era review policy in effect (playbook §1.8). Dogfood deferred for logistics (absolute commitment — tracked in `DOGFOOD.md` next to `context_files/`, kept current at every merge). **Order per the 2026-07-19 strategic review:** PR-DX3 (eval loop in CI) → Feature W SOTA note → W1 contract. Parked for the dogfood era: PR-M3 (signed, playbook §1), M-tasks, T, V; K/tree + memory-informed M-tasks additionally blocked by the sequencing rule until DOGFOOD item 2 closes. Per-PR contracts live in the playbook; this file stays the backlog-level view. New agent conversations start with the playbook's Handoff Protocol (§6).
